...
class SourceSignal[V](v: Option[V]) extends Signal[V](v) {
  ...
  /** A version of the `publish` method which takes the implicit execution context for dispatching.
    ...
    */
  @targetName("twobang")
  inline def !!(value: V)(using ec: ExecutionContext): Unit = publish(value, ec)
  ...
  /** Changes the value of the signal by applying the given function to the current value.
    ...
    *         false otherwise.
    */
  inline def mutate(f: V => V): Boolean = update(_.map(f))
  ...
  /** Changes the value of the signal by applying the given function to the current value.
    ...
    *         false otherwise.
    */
  inline def mutate(f: V => V, ec: ExecutionContext): Boolean = update(_.map(f), Some(ec))
...
private[signals3] object SourceSignal {
