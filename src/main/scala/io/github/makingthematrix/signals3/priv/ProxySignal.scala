...
private[signals3] object ProxySignal {
  @static class RecoverSignal[V](source: Signal[V], recover: Throwable => Option[V])
  ...
  @static class RecoverWithSignal[V](source: Signal[V], recoverWith: PartialFunction[Throwable, Option[V]])
    extends ProxySignal[V](source) {
  ...
  @static class ScanSignal[V, Z](source: Signal[V], zero: Z, f: (Z, V) => Z) extends ProxySignal[Z](source) {
    value = Some(zero)
  ...
  @static class FilterSignal[V](source: Signal[V], predicate: V => Boolean) extends ProxySignal[V](source) {
    override protected def computeValue(current: Option[V]): Option[V] = source.value.filter(predicate)
  }

  @static class MapSignal[V, Z](source: Signal[V], f: V => Z) extends ProxySignal[Z](source) {
    override protected def computeValue(current: Option[Z]): Option[Z] = source.value.map(f)
  }

  @static class GroupedSignal[V](source: Signal[V], n: Int) extends ProxySignal[Seq[V]](source) {
  ...
  @static class GroupBySignal[V](source: Signal[V], groupBy: V => Boolean) extends ProxySignal[Seq[V]](source) {
    private val buffer = scala.collection.mutable.ArrayBuffer.empty[V]
  ...
  @static class IndexedSignal[V](source: Signal[V]) extends ProxySignal[V](source) with Indexed {
    value = source.value
  ...
  @static class DropSignal[V](source: Signal[V], drop: Int) extends ProxySignal[V](source) with Indexed {
  ...
  @static final class CloseableSignal[V](source: Signal[V]) extends ProxySignal[V](source) with Closeable {
    override protected def computeValue(current: Option[V]): Option[V] = if (!isClosed) source.value else current
  }

  @static final class TakeWhileSignal[V](source: Signal[V], p: V => Boolean) extends ProxySignal[V](source) with Finite[V] {
  ...
  @static class DropWhileSignal[V](source: Signal[V], p: V => Boolean) extends ProxySignal[V](source) {
  ...
  @static class CollectSignal[V, Z](source: Signal[V], pf: PartialFunction[V, Z]) extends ProxySignal[Z](source) {
    override protected def computeValue(current: Option[Z]): Option[Z] =
  ...
  @static final class FoldLeftSignal[V, Z](sources: Signal[V]*)(v: Z)(f: (Z, V) => Z) extends ProxySignal[Z](sources*) {
    override protected def computeValue(current: Option[Z]): Option[Z] =
      sources.foldLeft(Option(v))((mv, signal) => for a <- mv; b <- signal.value yield f(a, b))
  }

  @static final class PartialUpdateSignal[V, Z](source: Signal[V])(select: V => Z) extends ProxySignal[V](source) {
  ...
  @static class StreamSignal[V](source: Stream[V], v: Option[V] = None) extends Signal[V](v) {
  ...
  @static class SequenceSignal[V](sources: Signal[V]*) extends ProxySignal[Seq[V]](sources*) {
    override protected def computeValue(current: Option[Seq[V]]): Option[Seq[V]] = {
  ...
  @static class CombineSignal[V, Z, Y](vSignal: Signal[V], zSignal: Signal[Z], f: (V, Z) => Y)
